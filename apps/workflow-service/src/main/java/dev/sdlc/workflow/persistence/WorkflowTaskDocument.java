...
public record WorkflowTaskDocument(
        @Id String taskId, TaskType type, TaskStatus status, WorkflowScope scope, String idempotencyKey,
    ...
    public static WorkflowTaskDocument fromDomain(WorkflowTask task) {
        return new WorkflowTaskDocument(task.taskId(), task.type(), task.status(), task.scope(), task.idempotencyKey(),
                task.assigneeId(), task.leaseExpiresAt(), task.version(), task.createdAt(), task.updatedAt());
    }

    public WorkflowTask toDomain() {
        return new WorkflowTask(taskId, type, status, scope, idempotencyKey, assigneeId, leaseExpiresAt,
