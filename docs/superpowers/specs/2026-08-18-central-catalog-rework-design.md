...
1. Replace the current partial bundle (3 agents, 4 skills, 2 policies) with the complete catalog from the seven-repository spec: 13 agents, 33 skills, 19 instructions, 15 policies, 20 templates, evals, mcp catalog, and bundle manifest.
...
| SDD stage boundaries | [github/spec-kit](https://github.com/github/spec-kit) | MIT | requirement 閳?spec 閳?plan 閳?task 閳?archive phase boundaries |
...
| Frontend engineering checklists | [vercel-labs/agent-skills](https://github.com/vercel-labs/agent-skills) | no license file | CONCEPT ONLY 閳?no text copied |
| Agent Skills examples | [anthropics/skills](https://github.com/anthropics/skills) | no license file | CONCEPT ONLY 閳?no text copied |
...
```text
central/
閳规壕鏀?agents/                       # 13 .agent.md files
閳规壕鏀?skills/
閳? 閳规壕鏀?workflow/                  # start-epic, join-epic, change-epic, start-ticket*, resume-workflow*, import-pod-members*
閳? 閳规壕鏀?analysis/                  # analyze-code-context, grill-requirement, assess-api-compatibility
閳? 閳规壕鏀?design/                    # design-solution, plan-change, adr
閳? 閳规壕鏀?implement/                 # implement-task, java-development, web-development, ios-development, android-development
閳? 閳规壕鏀?test/                      # generate-tests, plan-manual-e2e, record-manual-e2e, review-accessibility, review-analytics-tagging
閳? 閳规壕鏀?review/                    # prepare-pr*, review-pr
閳? 閳规壕鏀?onboard/                   # onboard-repository, onboard-journey, sync-onboarding, analyze-http-call-graph
閳? 閳规柡鏀?sm/                        # analyze-epic-risk, prepare-standup, find-blockers, check-release-readiness, draft-jira-update
閳规壕鏀?instructions/                 # 19 .instructions.md files
閳规壕鏀?policies/                     # 15 policies (2 existing reworked)
閳规壕鏀?templates/                    # 20 templates
閳规壕鏀?evals/                        # at least 1 behavioral scenario per agent and per core skill
閳规壕鏀?mcp/catalog.json              # reworked from current
閳规壕鏀?manifests/bundle-manifest.json
閳规柡鏀?REFERENCES.md
```
...
1. `epic-delivery-analyst` 閳?epic intake, ticket matrix, journey gap analysis (read-only tools + workflow MCP)
2. `delivery-coordinator` 閳?Scrum Master view: stand-up, blockers, release readiness (read-only + workflow MCP)
3. `requirement-analyst` 閳?grills requirements with code evidence (reworked existing)
4. `code-context-analyst` 閳?AS-IS evidence pack from code/Onboarding (read-only)
5. `solution-architect` 閳?cross-repo design, API compatibility, ADRs (reworked existing)
6. `planner` 閳?implementation plan and repo task breakdown
7. `java-implementer` 閳?Java/Spring implementation under repo instructions
8. `web-implementer` 閳?web implementation with accessibility baseline
9. `ios-implementer` 閳?iOS implementation
10. `android-implementer` 閳?Android implementation
11. `test-designer` 閳?automated + manual E2E test generation
12. `accessibility-qa` 閳?WCAG 2.2 AA + VoiceOver/TalkBack checks
13. `pr-reviewer` 閳?read-only structured review findings; `model` uses a review-preference fallback chain (reworked existing)
...
Skill 閳?agent ownership and the workflow gate chain follow the run-first spec (M1閳ユ彈7). `grill-requirement` keeps the Socratic single-question pattern; `import-pod-members` keeps DRY_RUN閳妽onfirm閳墣PPLY.
...
- `packages/contracts/test/central-bundle.test.ts` and `customizations.test.ts` extended to assert: 13 agents present, 33 skills present with valid frontmatter, agent鈫抯kill references resolvable, `REFERENCES.md` exists with license column, no Claude-only fields anywhere.

**Count correction (2026-08-18):** the headline count "33 skills" was a typo 鈥?the 搂5 list contains 33 skills and all downstream artifacts use 33.
